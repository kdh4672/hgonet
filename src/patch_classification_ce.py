...
def train(train_dataset,valid_dataset):
    ...
    # model.load_state_dict(torch.load('/home/ubuntu/database/edge-connect/patch_models/.pth'))
    model.to(device)
    loss_fn = nn.CrossEntropyLoss() #criterian
    ...
    for epoch in range(epoch):
        ...
        for items,l_prob,r_prob in train_loader:
            ...
            # prob = prob.to(device)
            # l_prob , r_prob = l_prob.to(device) , r_prob.to(device)
            model.zero_grad()
            logits = model(items)
            ...
            l_gt = torch.argmax(l_prob, dim=1).flatten().to(device)
            r_gt = torch.argmax(r_prob, dim=1).flatten().to(device)
            ...
            loss = loss_fn(l_logits,l_gt) + loss_fn(r_logits,r_gt)
        ...
        with torch.no_grad():
            ...
            for items,l_prob,r_prob in valid_loader:
                ...
                loss_ = loss_fn(l_logits,l_gt) + loss_fn(r_logits,r_gt)
